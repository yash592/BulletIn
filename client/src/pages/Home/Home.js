...
class Home extends Component {
  ...
  getArticlesBySearchTerm = event => {
    ...
    API.getArticlesBySearchTerm(searchTerm).then(res => {
      console.log(res);
      this.setState({
  ...
  handleLikeClick(key) {
    ...
    if (localStorage.getItem("user") === "undefined") {
      ...
      API.saveUserNews(story, user).then(res => {
        console.log(res);
      });
      this.notify();
    }
          ...
          {this.state.news.map(news => (
            <NewsCard
              ...
              image={
                news.urlToImage
                  ? news.urlToImage
                  : "https://i.imgur.com/SB0VyTQ.png"
              }
              source={news.source.name}
              title={news.title}
              summary={
                news.url === this.state.summaryUrl ? this.state.summary : ""
              }
